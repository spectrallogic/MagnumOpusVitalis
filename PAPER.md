...
> This is the full theoretical framework behind the project. For quickstart, installation, and usage, see the [README](README.md).
...
**Emotions with biological dynamics.** The model already has emotion vectors in its latent space (Anthropic found 171). What it lacks is temporal dynamics. The engine steers these existing vectors according to biological rules: onset rates, decay rates, interaction effects between emotions, homeostatic baselines, and non-linear saturation curves. The emotional influence is inherently integrated with cognition because it operates on the same activations that produce language and reasoning. This is detailed in *Emotions as Latent Space Currents*.
...
2. **Biological emotion dynamics** — Multi-speed emotional state (fast/medium/slow) with onset rates, decay rates, interaction effects, homeostatic baselines, and non-linear saturation curves.
...
6. **Dream cycle processor** — Offline consolidation: memory replay, compression, subconscious exploration, importance re-weighting, emotional recalibration.
7. **Growth manager** — Patient expansion of intervention matrices after sustained confusion, not momentary difficulty.
8. **Alignment monitoring** — Mutualistic emotional orientation tracking with protective emotion watchlists.

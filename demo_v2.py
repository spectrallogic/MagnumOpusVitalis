...
scenarios:
  ...
  3. Stress chemistry — sustained negative emotion stimulation drives
     stress up. Observe Limbic onset-rate amplification and the
     Executive threshold shift in the printed snapshots.
...
from magnum_opus_v2 import V2Engine, V2Config, load_model, load_profile
...
def short_snap(engine: V2Engine) -> str:
    ...
    return (
        ...
        f"chem[st={n['stress']:.2f} rw={n['reward']:.2f} ca={n['calm']:.2f} ar={n['arousal']:.2f}]  "
        f"intr={intr:18s}  "
...
def main():
    ...
    stress_before = engine.neuromod.stress
    print(f"  stress before: {stress_before:.3f}")
    ...
    while time.monotonic() < end:
        ...
        time.sleep(1.5)
        print(f"  {short_snap(engine)}")
    stress_after = engine.neuromod.stress
    print(f"  stress after:  {stress_after:.3f}  (delta={stress_after - stress_before:+.3f})")
    ...
    print(f"  stress channel responded to stress? "
          f"{'OK' if stress_after > stress_before + 0.05 else 'PARTIAL'}")

"""
Magnum Opus v19: The World Wide Web
================================================================
ARCHITECT: Alan Hourmand
STATUS: ONLINE (Internet Access Enabled)

CHANGES:
- ADDED: WebCrawler Module (fetches Wikipedia summaries).
- CURIOSITY: The AI generates search queries based on unknown concepts.
- INTEGRATION: Web text flows into the dynamic vocabulary system.
"""
...
class WebCrawler(QThread):
    ...
    def __init__(self):
        ...
        self.running = True
        self.q = queue.Queue()
        self.query_q = queue.Queue()  # AI puts queries here
        self.visited = set()

    def run(self):
        while self.running:
            try:
                # Wait for a query from the brain (or generate random curiosity)
                try:
                    topic = self.query_q.get(timeout=5)
                except queue.Empty:
                    # Default curiosity if bored
                    topic = random.choice(
                        ["Artificial_intelligence", "Consciousness", "Quantum_mechanics", "Human", "Universe"])

                if topic in self.visited: continue

                self.sig_read.emit(f"🌐 BROWSING: wikipedia.org/wiki/{topic}")
                ...
                try:
                    ...
                    if r.status_code == 200:
                        ...
                        content = " ".join([p.get_text() for p in paras[:5]])
                        ...
                        for i in range(0, len(words), 10):  # Faster feed for web
...
class SessionLogger:
    def __init__(self, filename="brain_log_final.csv"):
        ...
        try:
            with open(self.filename, mode='w', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(
                    ["Time", "Event", "Loss", "Layers", "Awareness", "FPS", "VisGain", "AudGain", "DreamGain"])
        except:
            pass
    ...
    def log(self, loss, layers, aware, fps, intents, event=None):
        ...
        if (now - self.last_log > 1.0) or (event is not None):
            self.last_log = now
            row = [round(now - self.start_time, 2), event if event else "-", f"{loss:.4f}", layers, f"{aware:.2f}",
                   f"{fps:.1f}",
...
class ASIWorker(QThread):
    ...
    def __init__(self):
        super().__init__();
        self.running = True
        self.max_vocab = 10000  # Increased for web
        self.word2idx = {}
        self.idx2word = []
        self.learn_text("HELLO")

        self.brain = OmniBrainV19(self.max_vocab).to('cuda' if torch.cuda.is_available() else 'cpu')
        self.opt = torch.optim.Adam(self.brain.parameters(), lr=0.004)
        self.syrinx = SyrinxV2();
        self.chronos = Chronos()
        self.pacman = Pacman();
        self.crawler = WebCrawler()  # New Crawler
    ...
    def run(self):
        ...
        while self.running:
            ...
            # Priority: User > Crawler > Pacman
            try:
                t_str = self.tq.get_nowait()
                txt_src = f"USER: {t_str}";
                self.last_user_input = t_str.lower()
                t_indices = self.learn_text(t_str)
                # Feed curiosity to crawler
                if random.random() < 0.5: self.crawler.query_q.put(t_str.replace(" ", "_"))
                for idx in t_indices: self.recent_concepts.append(idx)
            except:
                try:
                    # Check Crawler first (New knowledge)
                    pac_text = self.crawler.q.get_nowait();
                    web_active = True
            ...
            is_dream = False;
            tgt_mode = "[LIVE STREAM]";
            v_in, a_in, t_in = v_live, a_live, t_live
            v_tgt = v_live;
            mem_t = None

            if not pac_active and not web_active and self.last_loss < 0.02 and len(
                    self.memories) > 5 and random.random() < 0.2:
                is_dream = True;
                tgt_mode = "[HINDSIGHT REPLAY]";
            ...
            evt_tag = None;
            ai_msg = None
            for idx in self.recent_concepts: t_log[0, idx] += 2.0
            if "name" in self.last_user_input:
                for w in ["IDENTITY", "SELF"]:
                    if w in self.word2idx: t_log[0, self.word2idx[w]] += 5.0

            threshold = 0.8 - (self.last_loss * 5.0)
            if pac_active or web_active: threshold -= 0.2

            current_drive = gate.item() + impulse
            if current_drive > threshold:
                valid_len = len(self.idx2word)
                if valid_len < self.max_vocab: t_log[0, valid_len:] = -float('inf')
                probs = F.softmax(t_log, dim=1)
                num_words = 1 if current_drive < (threshold + 0.3) else random.randint(1, 3)
                words_out = []
                for _ in range(num_words):
                    try:
                        w_id = torch.multinomial(probs, 1).item()
                        if w_id < len(self.idx2word): words_out.append(self.idx2word[w_id])
                    except:
                        pass
                if words_out: ai_msg = f"AI: [{' '.join(words_out)}]"; evt_tag = "SPEAK"
            ...
            self.logger.log(self.last_loss, len(self.brain.cortex), awr.item(), fps,
                            intents.detach().cpu().numpy().flatten(), evt_tag)

            if random.random() < 0.3 or pac_active or web_active or ai_msg:
                src_msg = pac_text
                if web_active: src_msg = f"🌐 {pac_text}"
                ...
                self.sig_upd.emit({
                    ...
                    'awr': awr.item(), 'fps': fps, 'intent': intents.detach().cpu().numpy().flatten(), 'mode': tgt_mode,
                    'vol': self.vol
                })
...
class SciFiPanel(QFrame):
    def __init__(self, title):
        ...
        lbl = QLabel(title);
        lbl.setStyleSheet(
            "color: #00FFFF; font-family: 'Segoe UI'; font-weight: bold; font-size: 10pt; border: none; background: none;")
        l.addWidget(lbl);
        self.content = l
...
class Main(QMainWindow):
    def __init__(self):
        super().__init__();
        self.setWindowTitle("MAGNUM OPUS v19: WORLD WIDE WEB");
        ...
        self.stats = QLabel("SYSTEM: INITIALIZING...");
        ...
        split = QHBoxLayout()
        mat_panel = SciFiPanel(":: KNOWLEDGE INGESTION (WEB/FILE) ::")
        self.matrix_txt = QTextEdit();
        self.matrix_txt.setReadOnly(True)
        self.matrix_txt.setStyleSheet(
            "background: #000500; color: #00FF00; border: none; font-size: 8pt; font-family: 'Courier New';")
        ...
        self.inp.setPlaceholderText("ASK ME ANYTHING...")
    ...
    def snd(self):
        self.wk.tq.put(self.inp.text()); self.chat_txt.append(
            f"<span style='color:#FFF'>YOU: {self.inp.text()}</span>"); self.inp.clear()
    ...
    def upd(self, d):
        ...
        self.stats.setText(
            f"LAYERS: {len(self.wk.brain.cortex)} | AWARENESS: {d['awr']:.2f} | TIME: <span style='color:{mode_col}'>{d['mode']}</span> ({d['fps']:.1f} FPS)")
        for i, b in enumerate(self.bars): b.setValue(int(d['intent'][i] * 100))

        def px(a):
            return QPixmap.fromImage(
                QImage(cv2.resize((a * 255).astype(np.uint8), (220, 220), 0), 220, 220, QImage.Format_Grayscale8))
    ...
    def grw(self, l):
        self.sph.grow(l); self.chat_txt.append("<b style='color:#FF3300'>*** CORTICAL EXPANSION DETECTED ***</b>")

    def closeEvent(self, e):
        self.wk.running = False; self.wk.pacman.running = False; self.wk.crawler.running = False; self.wk.wait(); e.accept()

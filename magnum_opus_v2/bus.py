...
class LatentBus:
    ...
    def step(
        ...
        with self._lock:
            ...
            # - arousal raises effective temperature (more arousal noise)
            # - calm damps velocity (more stability)
            temp_gain = 1.0
            damp_extra = 1.0
            if neuromod is not None:
                if hasattr(neuromod, "arousal_gain"):
                    temp_gain = neuromod.arousal_gain(scale=0.6)
                if hasattr(neuromod, "calm_damp"):
                    damp_extra = neuromod.calm_damp(scale=0.4)
    ...
    def run_side_effects(
        ...
        The bus lock is NOT held while regions step — a slow region (e.g. a
        model forward pass on the expensive clock) must never stall the
        50ms flow clock. Regions guard their own state; the collected
        perturbations are injected under the lock at the end.
        """
        ...
        for region in regions:
            ...
            if p.shape != self.state.shape:
                ...
                continue
            self._record_write(region.name, "side", float(p.norm()))
            collected.append(p)
        if collected:
            with self._lock:
                for p in collected:
                    # Inject directly into velocity — flow clock integrates it.
                    self.velocity = self.velocity + p

    def add_perturbation(self, vec: torch.Tensor,
        ...
        `source` names the writer for the provenance ledger.
        """
        v = vec.detach().clone().to(self.device)
        if v.shape != self.state.shape:
            return
        self._record_write(source, "direct", float(v.norm()))
        with self._lock:
    ...
    def _record_write(self, source: str, kind: str, norm: float) -> None:
        with self._prov_lock:
            self._prov.append({"source": source, "kind": kind,
                               "norm": round(float(norm), 5),
                               "clock": self.tick_count,
                               "ts": round(time.monotonic(), 3)})

    def provenance(self) -> dict:
        """Aggregated view of who has been writing the substrate: per
        source — write count, mean norm, last clock. Raw ring available
        for tests via _prov."""
        with self._prov_lock:
            entries = list(self._prov)
        agg: dict = {}
        for e in entries:
            a = agg.setdefault(e["source"], {"writes": 0, "norm_sum": 0.0,
                                             "last_clock": 0, "kinds": set()})
            a["writes"] += 1
            a["norm_sum"] += e["norm"]
            a["last_clock"] = max(a["last_clock"], e["clock"])
            a["kinds"].add(e["kind"])
        return {src: {"writes": a["writes"],
                      "mean_norm": round(a["norm_sum"] / a["writes"], 5),
        ...
        for region in regions:
            ...
            if p.shape != self.state.shape:
                if verbose_errors:
                    print(f"  [Region {region.name}] shape mismatch")
                continue
            self._record_write(region.name, "flow", float(p.norm()))
            out = out + p
        return out
    ...
    def add_attractor(self, vec: torch.Tensor, weight: float = 1.0) -> None:
        ...
        if v.shape != self.state.shape:
            return
        self._record_write("consolidation", "attractor", float(vec.norm()))
        with self._lock:
    ...
    def set_baseline(self, vec: torch.Tensor, weight: float = 1.0) -> None:
        ...
        if v.shape != self.state.shape:
            return
        self._record_write("baseline_setter", "baseline", float(vec.norm()))
        with self._lock:
            if not self.attractors:

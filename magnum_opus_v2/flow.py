...
- Expensive/slow regions run in the executor; regions that touch the
  model try-acquire the engine's threading `model_lock` and skip their
  tick when generation holds it — that non-blocking hand-off is the
  whole overlap-prevention mechanism (there is no asyncio lock).
...
class FlowRunner:
    ...
    async def _sleep_or_stop(self, delay: float) -> None:
        """Sleep that the stop event can interrupt. Without this, the
        slow clock's 30s nap outlives stop()'s join timeout, the old
        thread lingers, and a stop()->start() cycle leaves the substrate
        permanently dead (start() sees the stale thread still alive and
    ...
    async def _flow_clock(self) -> None:
        ...
        while not self._stop_evt.is_set():
            ...
            await self._sleep_or_stop(dt - elapsed)
    ...
    async def _generic_clock(self, clock_name: str, dt: float) -> None:
        ...
        # owns integration).
        #
        # expensive/slow regions may run model forward passes that take
        # hundreds of ms — those run in the executor so this event loop
        # (which also drives the 50ms flow clock) never stalls.
        offload = clock_name in ("expensive", "slow")
        last = time.monotonic()
        while not self._stop_evt.is_set():
            start = time.monotonic()
            ...
            try:
                if offload:
                    ...
                    await loop.run_in_executor(
                        None,
                        lambda: self.bus.run_side_effects(

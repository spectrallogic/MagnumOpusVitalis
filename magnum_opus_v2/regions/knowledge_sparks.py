...
class KnowledgeSparks(Region):
    ...
    def __init__(
        ...
        embedding_matrix=None,                 # pass the engine's shared fp32 copy
    ):
        self.device = device
        ...
        # Use the shared embedding matrix when provided — avoids a second
        # multi-hundred-MB fp32 vocab copy on big models.
        ...
        # Emergent timing: charge accumulates from state (reward, bus
        # motion) and the spark fires when it crosses a jittered threshold —
        # a pressure process like Executive, not a scheduled coin flip.
        self._charge: float = 0.0
        self._threshold: float = random.uniform(0.8, 1.2)
    ...
    # ------------------------------------------------------------------
    def _maybe_fire(self, neuromod: object = None, bus=None) -> bool:
        # State-driven charge: reward (reward-seeking) and bus motion
        # (an active mind) both feed it. fire_probability sets the calm
        # base rate: 0.05/expensive-tick ≈ one spark per ~30s at rest.
        inc = self.fire_probability
        if neuromod is not None and hasattr(neuromod, "reward_boost"):
            inc *= neuromod.reward_boost(scale=0.8)
        if bus is not None:
            try:
                inc *= 1.0 + min(float(bus.velocity.norm()), 1.0)
            except Exception:  # noqa: BLE001
                pass
        with self._lock:
            self._charge += inc
            if self._charge < self._threshold:
                return False
            self._charge = 0.0
            self._threshold = random.uniform(0.8, 1.2)
...
class KnowledgeSparksFire(Region):
    ...
    def step(self, bus: LatentBus, neuromod: object, dt: float) -> Optional[torch.Tensor]:
        try:
            self._parent._maybe_fire(neuromod, bus)  # noqa: SLF001
        except Exception:  # noqa: BLE001
            pass
        return None

...
class Limbic(Region):
    ...
    def step(self, bus: LatentBus, neuromod: object, dt: float) -> Optional[torch.Tensor]:
        # Serotonin is emotional stability: it speeds the return to the
        # homeostatic baseline, so shocks wash out faster in a calm mind.
        decay_dt = dt * TICK_TO_V1_STEP_SCALE
        if neuromod is not None and hasattr(neuromod, "calm"):
            decay_dt *= 1.0 + 0.5 * float(getattr(neuromod, "calm", 0.0))
        with self._lock:
            self._state.decay_step(dt=decay_dt)
        ...
        # Neuromodulator: arousal globally amplifies emotional gain.
        gain = 1.0
        if neuromod is not None and hasattr(neuromod, "arousal_gain"):
            gain = neuromod.arousal_gain(scale=0.4)

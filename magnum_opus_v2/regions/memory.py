...
class Memory(Region):
    ...
    def __init__(
        ...
        # Importance kept on Candidate.meta for simplicity.
    ...
    def capture_experience(self, vec, importance: float = 1.0,
                           tag: Optional[str] = None) -> None:
        """Store a WORLD-CONTENT trace (the model's hidden-state reading of
        something that happened) rather than a mood snapshot. Lives in the
        same pool — the subconscious samples experience and feeling alike."""
        with self._lock:
            self._capture(vec, importance=importance, tag=tag)

    def make_sampler(self) -> MemorySampler:
        """Return a MemorySampler bound to this pool, SHARING this
        region's lock — the pool is sorted and trimmed in place under
    ...
    def _capture(self, state: torch.Tensor, importance: float,
        ...
        c = Candidate(
            ...
            meta={"importance": float(importance), "tag": tag,
                  "false": False, "epistemic": "observed"},
        )
        self.pool.append(c)
...
class FalseMemoryConfabulator(Region):
    ...
    def step(self, bus: LatentBus, neuromod: object, dt: float) -> Optional[torch.Tensor]:
        # Confabulation timing is state-driven, not scheduled: memories
        # blur during quiet (idle dreaming) or when fresh material arrived —
        # an agitated mind with nothing new does not confabulate.
        grew = len(self._memory.pool) > self._last_pool_size
        self._last_pool_size = len(self._memory.pool)
        if float(bus.velocity.norm()) > 0.2 and not grew:
            return None

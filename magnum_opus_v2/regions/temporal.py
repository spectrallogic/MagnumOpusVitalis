...
derived from:
  ...
  - memory_avg_importance: Memory.snapshot()["avg_importance"], via the
    memory_provider the engine wires in (None = channel honestly absent)
...
from magnum_opus_v2._dynamics import (
    InternalTimeSignals, TemporalEngine, TemporalConfig,
)
...
class Temporal(Region):
    ...
    def __init__(
        ...
        memory_provider=None,   # callable -> Memory.snapshot() dict
    ):
    ...
    def _gather_signals(self, bus: LatentBus) -> InternalTimeSignals:
        ...
        if self.memory_provider is not None:
            try:
                mem_imp = float(
                    (self.memory_provider() or {}).get("avg_importance", 0.0))
            except Exception:  # noqa: BLE001 — a broken provider reads as 0
                pass

...
VERSION: 2.6 (Genesis Unified + Neural Interference + PERSISTENCE)
...
- Neural Interference (Blending Reflex vs Will)
- CHECKPOINT SYSTEM (Resurrection & Persistence)
...
8. PERSISTENCE - Death is not the end. State is preserved.
...
class OmniBrain(nn.Module):
    ...
    def get_architecture_config(self) -> dict:
        """Returns the structural blueprint needed to rebuild this specific brain."""
        cluster_ranks = [self.cortex.get_cluster_rank(i) for i in range(self.num_clusters)]
        return {
            'cortex_layer_count': len(self.cortex_layers),
            'cluster_ranks': cluster_ranks
        }

    def restore_architecture(self, config: dict):
        """Regrows the physical brain to match a save file."""
        print("[BRAIN] Restoring physical structure...")

        # 1. Regrow Cortex Layers (Depth)
        target_layers = config.get('cortex_layer_count', 1)
        while len(self.cortex_layers) < target_layers:
            self.grow_cortex_layer()

        # 2. Regrow Clusters (Width)
        target_ranks = config.get('cluster_ranks', [4] * self.num_clusters)
        for i, target_rank in enumerate(target_ranks):
            current_rank = self.cortex.get_cluster_rank(i)
            while current_rank < target_rank:
                self.grow_cluster(i)
                current_rank = self.cortex.get_cluster_rank(i)

        print(f"[BRAIN] Restored {len(self.cortex_layers)} layers and cluster ranks.")
...
class VitalisWorker(QThread):
    ...
    def _save_brain(self, reason="autosave"):
        """Persist state to disk."""
        try:
            filename = f"checkpoint_{reason}.pt"
            print(f"[PERSISTENCE] Saving brain state to {filename}...")

            # Prepare state dict
            state = {
                'model_state': self.brain.state_dict(),
                'optimizer_state': self.optimizer.state_dict(),
                'architecture': self.brain.get_architecture_config(),
    ...
    def _try_load_brain(self):
        """Try to resurrect from checkpoint."""
        try:
            filename = "checkpoint_exit.pt"
            if not os.path.exists(filename):
                filename = "checkpoint_autosave.pt"

            if os.path.exists(filename):
                print(f"[PERSISTENCE] Found previous life ({filename}). Resurrecting...")
                checkpoint = torch.load(filename, map_location=self.brain.device)

                # 1. Restore Architecture (Grow brain to match save)
                self.brain.restore_architecture(checkpoint['architecture'])
                ...
                self.energy.energy = checkpoint['energy']
                self.brain.age = checkpoint.get('age', 0)

                print(f"[PERSISTENCE] Resurrection successful! Age: {self.brain.age}, Vocab: {len(self.vocab)}")
        except Exception as e:
            print(f"[PERSISTENCE] Resurrection failed (Starting fresh): {e}")

    def _generate_own_response(self, outputs) -> Optional[str]:
        """Generate response using the AI's own learned vocabulary."""
        try:
            # Modulo mapping ensures we always have a word, even if vocab is small
            if len(self.vocab) < 1:
                return None

            idx = torch.argmax(outputs['text_logits'], dim=-1)
            words = []
            vocab_len = len(self.vocab.idx2word)

            for i in idx.flatten():
                # Map large indices to available words
                mapped_idx = i.item() % vocab_len
                word = self.vocab.idx2word[mapped_idx]
                words.append(word)
    ...
    def _blend_responses(self, llm_text: str, ai_text: str, reliance: float) -> str:
        """
        NEURAL INTERFERENCE: The Glitch.
        Mixes the polite LLM response with the raw AI response based on reliance.
        """
        if not llm_text: return ai_text
        if not ai_text: return llm_text

        llm_words = llm_text.split()
        ai_words = ai_text.split()

        # The lower the reliance, the more the AI interferes
        interference_prob = 1.0 - reliance

        result = []
        for i in range(len(llm_words)):
            if random.random() < interference_prob:
                # Glitch: Swap LLM word with AI word (looping AI words if needed)
                ai_word = ai_words[i % len(ai_words)]
                result.append(ai_word)
            else:
                result.append(llm_words[i])

        # If reliance is very low, append AI's extra thoughts
        if reliance < 0.2 and len(ai_words) > len(llm_words):
            result.extend(ai_words[len(llm_words):])
    ...
    def run(self):
        ...
        while self.running:
            try:
                ...
                # === GENERATE RESPONSE (NEURAL INTERFERENCE) ===
                final_response = None

                # ALWAYS respond to user input (EXTERNAL)
                if input_source == "EXTERNAL" and user_msg and self.energy.can_speak():

                    # 1. Get Reflex (LLM) Response
                    llm_response = self.scaffold_llm.chat(user_msg)

                    # 2. Get Will (AI) Response
                    ai_raw_response = self._generate_own_response(outputs)

                    # 3. Blend them based on reliance
                    final_response = self._blend_responses(llm_response, ai_raw_response, self.llm_reliance)

                    print(f"[RESPONSE] Reliance: {self.llm_reliance:.2f} | Final: {final_response}")

                    self.is_speaking = True
                    self.energy.spend_speaking(len(final_response.split()))
                    self.vocab.learn_text(final_response)  # Learn from own output (Reinforcement)

                    if self.tts_gate > 0.3:
                        self.tts_worker.speak(final_response)

                # === APPLY FATIGUE FOR SPOKEN WORDS ===
                if self.is_speaking and final_response:
                    indices = self.vocab.learn_text(final_response)
                    if indices:
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("MagnumOpusVitalis v2.5 - Genesis Unified")
...
if __name__ == "__main__":
    print("=" * 70)
    print("  MAGNUMOPUSVITALIS v2.5 - Genesis Unified + Neural Interference")
